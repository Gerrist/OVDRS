...
const parseCHBXMLtoJSON = () => {
    ...
    fs.readFile(`${__dirname}/${constantLatestStringXML}`, function (err, data) {
        ...
        parser.parseString(data, function (err, result) {
            ...
            console.log("Writing " + constantLatestString);


            ///////////////////////////////
            // write away current and latest version, for versioning
            fs.writeFile(
                `tmp/${constantLatestString}.json`,
                JSON.stringify(
            ...
            console.log("Writing 2 " + latestCHBExportName());

            fs.writeFile(
                `tmp/${latestCHBExportName()}.json`,
                JSON.stringify(
                    // Only include results from Amsterdam
                    filteredResultsAllProperties
                ),
            ...
            console.log(`Writing to tmp/${constantLatestString}-quays-selection.json`);

            fs.writeFile(
                `tmp/${constantLatestString}-quays-selection.json`,
                JSON.stringify(
                    // Only include results from Amsterdam
            ...
            console.log(`tmp/${latestCHBExportName()}-quaysselection.json`);

            fs.writeFile(
                `tmp/${latestCHBExportName()}-quaysselection.json`,
                JSON.stringify(
                    // Only include results from Amsterdam
                    quaysSelectionAmsterdam
                ),
            ...
            console.log("quaysAndPropsSelectionAmsterdam")
            ...
            console.log('Writing to' + `'tmp/${constantLatestString}-quays-and-props-selection.json'`)

            // write away current and latest version, for versioning
            fs.writeFile(
                `tmp/${constantLatestString}-quays-and-props-selection.json`,
                JSON.stringify(
                    // Only include results from Amsterdam
            ...
            fs.writeFile(
                `tmp/${latestCHBExportName()}-quays-and-props-selection.json`,
                JSON.stringify(
                    // Only include results from Amsterdam
                    quaysAndPropsSelectionAmsterdam
                ),
